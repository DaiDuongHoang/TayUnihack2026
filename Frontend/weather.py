...
class WeatherPage:
    ...
    def _render_large_forecast_chart(
        self, hourly_rows: list[dict[str, Any]], location_label: str
    ) -> None:
        st.markdown("### 📈 Forecast Chart")
        st.caption(
            "🌡️ Use the day controls to move across today, tomorrow, and the day after."
        )
        st.markdown(
            f"""
        ...
        with c_prev:
            if selected_offset > 0:
                if st.button(
                    "⬅️ Previous Day", key="chart_prev_day", use_container_width=True
                ):
                    st.session_state.weather_chart_day_offset = max(
                        0, selected_offset - 1
                    )
        ...
        with c_next:
            if selected_offset < 2:
                if st.button(
                    "Next Day ➡️", key="chart_next_day", use_container_width=True
                ):
