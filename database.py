...
def reset_password(email: str, new_password: str) -> bool:
    ...
    with get_connection() as conn:
        user = conn.execute(
            'SELECT id, auth_provider FROM users WHERE username = ?', (normalized_email,)
        ).fetchone()
        if user is None or user['auth_provider'] != 'local':
            return False
        conn.execute(
...
def add_clothing_item(
    ...
    with get_connection() as conn:
        cur = conn.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (user_id, clean_name, resolved_category, cloth_type, color, image_data, min_temp, max_temp),
        )
        conn.commit()
...
def update_clothing_item(
    ...
    with get_connection() as conn:
        cur = conn.execute(
            ...
            """,
            (clean_name, resolved_category, cloth_type, color, min_temp, max_temp, clothing_id, user_id),
        )
        conn.commit()
        return cur.rowcount > 0
...
def add_new_clothes(user_id: int, category: str, color: str | None, img_path: str | None) -> None:
    ...
    with get_connection() as conn:
        conn.execute(
            ...
            (
                ...
                category, color, img_path, min_t, max_t,
            ),
        )
        conn.commit()
...
def get_user_catalog(email: str) -> dict[str, list[dict[str, object]]]:
    user_id = _resolve_user_id(email)
    catalog: dict[str, list[dict[str, object]]] = {cat: [] for cat in FRONTEND_CATEGORIES}
